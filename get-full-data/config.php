...
if ($wpdb->insert('full_fetch_data',array('database_name'=>$databaseName,'separator_style'=>$separa,'created_time'=>$date))===FALSE) {
...
$separa=substr($separa,0,1);
if ($wpdb->update('full_fetch_data',array('database_name'=>$databaseName,'separator_style'=>$separa,'created_time'=>$date) ,array( 'id' => $_POST['id_update'] ))===FALSE) {
	echo "<div class='error'><p>Mm !: Must be something wrong with your input fields,please try again</p></div>";
...
<h2 class="bigenogh">Configuration Data-fetch</h2>
<?php 
	$result=$wpdb->get_results("SELECT * FROM full_fetch_data ORDER BY ID DESC LIMIT 1");
...
<div class="wp-list-table widefat fixed posts" style="width:60%;background:#fff;border: 1px solid #e1e1e1;;padding:20px;">
 <form action="?page=get-full-data/config.php" method="POST">
