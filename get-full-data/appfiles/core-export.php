...
/*****Get Value From DataBase full_fetch_data*****/

$getdatabase_name=$wpdb->get_results("SELECT * FROM full_fetch_data ORDER BY ID DESC LIMIT 1");
...
<div class="separator" style="display:none;"><?php echo $sepa ?></div>

<h2 class="admintitle">Get Your Data List </span></span></h2>
<?php if ($checkmefirst==true): ?>
    <div class="subsubsub">
<div class="wp-list-table widefat fixed posts" style="background:#fff;border: 1px solid #e1e1e1;;padding:20px;">
<h3 style="color:#999" >Your Data list is ready ! <span class="admincount">(<?php echo $total ?> items)<span>&nbsp;was found</h3 >
<form action='?page=data-fetch/telecharger.php' method='post'>
<input type="hidden" value="yoitme" name="role">

<a type='submit' id="export" class='button action'>Yeah ! Download</a></form>
</div>
